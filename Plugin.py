...
class MyMusic(Plugin_Generique):
    ...
    def le_menu_feuille(self, numeroItemSelectionBranche):
        ...
        if numeroItemSelectionBranche == 0 or \
            ...
            #self.frameRandomPlay.doModal()
            # self.frameRandomPlay.show()

            # if not self.origine.can_randomplay:
            #     # the server is not able  -> print not possible
            #     self.origine.functionNotPossible(self.origine.listMenu_Fleur)
            #     return
            #
            # self.connectInterface()
            # self.get_ident_server()
            ...
            # self.origine.listMenu_Feuilles.addItem(itemtampon)
            # itemtampon = xbmcgui.ListItem()
            ...
            # listenrequete = self.playerid + ' status 0 100 subscribe:10'
            # self.InterfaceCLI.sendtoCLISomething(listenrequete)
            # reponse = self.InterfaceCLI.receptionReponseEtDecodage()
            #
            # if 'subscribe:' in reponse:
            #
            #     if 'playlist_cur_index:' in reponse:
            #         # ask what to do : new one or continue
            #         '''
            #         choix = xbmcgui.Dialog().select(heading= 'continue or new one',
            #                                         list= ['Continue with the actual playlist', \
            #                                                'Play a new random playlist'])
            #
            #         if choix < 0:
            #             return
            #
            ...
            #             self.InterfaceCLI.sendtoCLISomething(requete)
            #             reponse = self.InterfaceCLI.receptionReponseEtDecodage()
            #             if not 'randomplay|tracks' in reponse:
            #                 self.functionNotYetImplemented()
            #                 return
            #             # always following by filling the subscribe :
            #             reponse = self.InterfaceCLI.receptionReponseEtDecodage()
            #         '''
            #         itemtampon = xbmcgui.ListItem()
            #         itemtampon.setLabel('Continue with the actual playlist')
            #         self.origine.listMenu_Feuilles.addItem(itemtampon)
            #         itemtampon = xbmcgui.ListItem()
            #         itemtampon.setLabel('Play a new random playlist tracks')
            #         self.origine.listMenu_Feuilles.addItem(itemtampon)
            #         itemtampon = xbmcgui.ListItem()
            #         itemtampon.setLabel('Play a new random playlist Albums')
            #
            #     else:
            #         requete = self.playerid + ' randomplay tracks'
            #         self.InterfaceCLI.sendtoCLISomething(requete)
            ...
            #         reponse = self.InterfaceCLI.receptionReponseEtDecodage()
            #
            #     self.frameRandomPlay = FramePlaylist.PlaylistPlugin('Now playing random tracks')
            #     self.frameRandomPlay.show()
            #     # todo : change the listmenuallArtist for a listmenuRandomtracks
            #     # because to keep full the listmenuallartist with list artists
            #
            #     self.frameRandomPlay.setFocus(self.frameRandomPlay.listMenu_playlist)
            #     self.frameRandomPlay.listMenu_playlist.reset()
            #     self.frameRandomPlay.listMenu_playlist.setVisible(True)
            #
            #     compteur = 0
            #     self.frameRandomPlay.update_coverbox(lmsip=self.lmsip, lmswebport=self.lmswebport, playerid=self.playerid, compteur=compteur)
            ...
            #             break
            #
            #         if self.frameRandomPlay.mustQuit:
            #             self.frameRandomPlay.close()
            #             break
            #         #while reponse:
            ...
            #         self.frameRandomPlay.update_coverbox(lmsip=self.lmsip, lmswebport=self.lmswebport, playerid=self.playerid, compteur=compteur)
            #
            #         poubelle, atraiter = reponse.split('subscribe:10|')
            #
            #         '''
            #         exemple :
            ...
            #         '''
            #         try:
            #             textC = atraiter  # on conserve la deuximème trame après suscribe...
            #         except IndexError:
            #             break
            #             #pass
            ...
            #
            #         try:
            #             playlist_cur_index = dico['playlist_cur_index']
            #             nbre_de_piste = dico['playlist_tracks']
            #         except:
            #             pass
            #
            ...
            #
            #         self.frameRandomPlay.listMenu_playlist.selectItem(int(playlist_cur_index))
            #         xbmc.log('heure avant : ' + str(time.time()),xbmc.LOGNOTICE)
            #         reponse = self.InterfaceCLI.receptionReponseEtDecodage()
            #         xbmc.log('heure après : ' + str(time.time()),xbmc.LOGNOTICE)
            #
            #
            # else:
            ...
            #     return
            #
            # requete = self.playerid + ' status 0 2 subscribe:-'
            # self.InterfaceCLI.sendtoCLISomething(requete)
            # reponse= self.InterfaceCLI.receptionReponseEtDecodage()
            #
            # while not 'subscribe:-' in reponse:
            #     self.InterfaceCLI.sendtoCLISomething(requete)
            #     reponse= self.InterfaceCLI.receptionReponseEtDecodage()
            #
            # # end if random mix
